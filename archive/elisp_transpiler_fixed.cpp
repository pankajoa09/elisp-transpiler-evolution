    ...
    if (sexpr->type == "defun") {
        ...
        for (const auto& arg : get<shared_ptr<SExpr>>(sexpr->args[0])->args) {
            if (holds_alternative<shared_ptr<SExpr>>(arg)) {
                const auto& param_expr = get<shared_ptr<SExpr>>(arg);
                if (!param_expr->args.empty() && holds_alternative<string>(param_expr->args[0])) {
                    params.push_back(var_type_to_string(get_type(sexpr->args.back())) + " " + get<string>(param_expr->args[0]));
                }
            }
        }
